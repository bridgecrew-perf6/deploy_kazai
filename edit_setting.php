...
if (isset($_GET['member_id'])) { // member の変更の場合
  ...
  $stmt = $pdo->prepare('SELECT * FROM household_member WHERE member_id = :member_id AND house_id = :house_id');
  ...
  $stmt = $pdo->prepare('SELECT * FROM household_room WHERE room_id = :room_id AND house_id = :house_id');
  ...
  $stmt = $pdo->prepare('SELECT * FROM household_category WHERE category_id = :category_id AND house_id = :house_id');
  $stmt->bindValue(':house_id', $_SESSION['house_id'], PDO::PARAM_INT);
