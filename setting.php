...
if (isset($_POST['name'])) {
  $name = sr($_POST['name']);
  $stmt = $pdo->prepare('INSERT INTO household_member(name, house_id, in_date) VALUES(:name, :house_id, sysdate())');
...
if (isset($_POST['room'])) {
  $room = sr($_POST['room']);
  $stmt = $pdo->prepare('INSERT INTO household_room(room, house_id, in_date) VALUES(:room, :house_id, sysdate())');
...
if (isset($_POST['category'])) {
  ...
  $stmt = $pdo->prepare('INSERT INTO household_category(category, house_id, in_date) VALUES(:category, :house_id, sysdate())');
...
// member 一覧を表示
$stmt = $pdo->prepare('SELECT * FROM household_member WHERE house_id = :house_id');
$stmt->bindValue(':house_id', $_SESSION['house_id'], PDO::PARAM_INT);
$status = $stmt->execute();
$member_view = '';
if ($status === false) {
...
// room 一覧を表示
$stmt = $pdo->prepare('SELECT * FROM household_room WHERE house_id = :house_id ORDER BY room ASC');
$stmt->bindValue(':house_id', $_SESSION['house_id'], PDO::PARAM_INT);
$status = $stmt->execute();
$room_view = '';
if ($status === false) {
...
// category 一覧を表示
$stmt = $pdo->prepare('SELECT * FROM household_category WHERE house_id = :house_id ORDER BY category ASC');
$stmt->bindValue(':house_id', $_SESSION['house_id'], PDO::PARAM_INT);
$status = $stmt->execute();
$category_view = '';
if ($status === false) {
