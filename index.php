...
if (isset($_POST['photo'])) {
  ...
  $stmt = $pdo->prepare('INSERT INTO household_list
      (photo, name, category, room, description, state, house_id, in_date)
    VALUES
      (:photo, :name, :category, :room, :description, "募集中", :house_id, sysdate())');
...
if ( $_SESSION['disp_mode'] === "all" ) {
  ...
  $stmt = $pdo->prepare('SELECT * FROM household_list WHERE house_id = :house_id ORDER BY item_id DESC');
  ...
  $select_view .= '<div><input type="radio" name="select" value="close" id="close"><label for="close">行き先決定済み</label></div>';
  $stmt = $pdo->prepare('SELECT * FROM household_list WHERE state = "募集中" AND house_id = :house_id ORDER BY item_id DESC');
  ...
  $stmt = $pdo->prepare('SELECT * FROM household_list WHERE state = "行き先決定済み" AND house_id = :house_id ORDER BY item_id DESC');
}
$stmt->bindValue(':house_id', $_SESSION['house_id'], PDO::PARAM_INT);
...
<html lang="ja">
  ...
  <body>
    <div class="title_area">
      ...
      <a href="logout.php">
        <img class="icon" src="image/logout.png" alt="設定" title="ログアウトする">
      </a>
